...
Returned by `PatientTimelineModule.get_permissions()`
(relative names; the registry namespaces them as `patient_timeline.<name>`).

| Permission | Allows | Required by |
|------------|--------|-------------|
| `patient_timeline.read` | _Describe what this allows._ | _List the endpoints._ |
...
1. Add the relative name to `get_permissions()` in
   `backend/app/modules/patient_timeline/__init__.py` (or `module.py`).
